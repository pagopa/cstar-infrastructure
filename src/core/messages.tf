...
module "event_hub" {
  ...
  location                      = var.location
  resource_group_name           = azurerm_resource_group.msg_rg[0].name
  ...
  private_dns_zones = {
    id   = [data.azurerm_private_dns_zone.eventhub_private_dns_zone.id]
...
resource "azurerm_private_endpoint" "evh-cstar-vnet-private-endpoint" {
  ...
  private_service_connection {
    ...
    private_connection_resource_id = module.event_hub[0].namespace_id
...
resource "azurerm_key_vault_secret" "event_hub_keys" {
  #for_each = module.event_hub[0].key_ids
