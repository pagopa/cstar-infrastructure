...
<policies>
    <inbound>
        ...
        <cache-lookup-value key="@((string)context.Variables["v_fiscalcode"] + "-getcustomer")" variable-name="getCustomerResponse" caching-type="external" />
        <choose>
    ...
    <outbound>
        <choose>
            <when condition="@(context.Response.StatusCode >= 200 &&  context.Response.StatusCode < 300)">
                ...
                <cache-store-value key="@((string)context.Variables["v_fiscalcode"] + "-getcustomer")" value="@(context.Response)" duration="86400" caching-type="external" />
            </when>
        </choose>
