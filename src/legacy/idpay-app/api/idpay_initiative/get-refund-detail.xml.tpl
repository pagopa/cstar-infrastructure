        ...
        <choose>
            <when condition="@(context.Response.StatusCode >= 200 &&  context.Response.StatusCode < 300)">
                <choose>
                            ...
                            <when condition="@(((IResponse)context.Variables["responsePDV"]).StatusCode == 200)">
                                <return-response>
                                    ...
                                    <set-body>@{
                                        ...
                                        body.Property("beneficiaryType").Remove();
                    ...
                    <when condition="@((string)context.Response.Body.As<JObject>(preserveContent: true)["beneficiaryType"] == "MERCHANT")">
                        <return-response>
                            <set-header name="Content-Type" exists-action="override">
                                <value>application/json</value>
                            </set-header>
                            <set-body>@{
                                JObject body = context.Response.Body.As<JObject>();
                                body.Add(new JProperty("fiscalCode", body.Property("merchantFiscalCode").Value));
                                body.Add(new JProperty("eventId", body.Property("externalId").Value));
                                body.Property("userId").Remove();
                                body.Property("id").Remove();
                                body.Property("externalId").Remove();
                                body.Property("merchantFiscalCode").Remove();
                                body.Property("beneficiaryType").Remove();
                                return body.ToString();
                            }</set-body>
                        </return-response>
