      ...
      "content": {
        ...
        "items": [
          ...
          },
          {
            ...
            "content": {
              "version": "KqlItem/1.0",
              "query": "let failed = AppTraces\n| where TimeGenerated {evaluation_window:query}\n| where AppRoleName == 'emdmessagecore'\n| where Message has \"[MESSAGE-CORE][SEND] Fiscal code check failed for recipient:\"\n| summarize FailedCount = count() by bin(TimeGenerated, 6h);\nlet passed = AppTraces\n| where TimeGenerated {evaluation_window:query}\n| where AppRoleName == 'emdmessagecore'\n| where Message has \"[MESSAGE-CORE][SEND] Fiscal code check passed for recipient:\"\n| summarize PassedCount = count() by bin(TimeGenerated, 6h);\nlet error = AppTraces\n| where TimeGenerated {evaluation_window:query}\n| where AppRoleName == 'emdmessagecore'\n| where Message has \"[MESSAGE-CORE][SEND] Error while checking fiscal code for recipient:\"\n| summarize ErrorCount = count() by bin(TimeGenerated, 6h);\nfailed\n| join kind=fullouter passed on TimeGenerated\n| join kind=fullouter error on TimeGenerated\n| project TimeGenerated, FailedCount, PassedCount, ErrorCount\n| order by TimeGenerated desc",
              "size": 0,
              "queryType": 0,
              "resourceType": "microsoft.operationalinsights/workspaces",
              "crossComponentResources": [
