...
resource "azapi_resource" "apim-thumbprint-retriever" {
  type      = "Microsoft.ApiManagement/service/policyFragments@2021-12-01-preview"
  name      = "idpay-thumbprint-retriever"
  parent_id = data.azurerm_api_management.apim_core.id

  body = jsonencode({
    properties = {
      description = "idpay-thumbprint-retriever"
      format      = "rawxml"
      value = templatefile("./api_fragment/thumbprint-retriever.xml", {
        ingress_hostname = var.ingress_load_balancer_hostname
      })
    }
  })
