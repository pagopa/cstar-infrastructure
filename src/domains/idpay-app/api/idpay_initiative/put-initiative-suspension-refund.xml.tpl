    ...
    <inbound>
        ...
        <choose>
          ...
          <when condition="@(((IResponse)context.Variables["responsePDV"]).StatusCode == 200)">
            ...
            <rewrite-uri template="@("/idpay/organization/"+((Jwt)context.Variables["validatedToken"]).Claims.GetValueOrDefault("org_id", "")+"/initiative/{initiativeId}/user/"+(string)((IResponse)context.Variables["responsePDV"]).Body.As<JObject>()["token"]+"/suspend")" />
          </when>
          <otherwise>
            <return-response>
