  ...
  "items": [
    {
      "type": 1,
      "content": {
        "json": "This workbook is intended to show the performance observed during the generation of the csv containing the reward notification to be exported by the organization",
        "style": "info"
      },
      "name": "text - 6"
    },
        ...
        "parameters": [
          {
            ...
            "value": {
              "durationMs": 300000,
              "endTime": "2023-02-17T11:33:00.000Z"
            }
          },
          {
          ...
          {
            "id": "6874b66c-6e15-4d72-8000-c5cef823af68",
            "version": "KqlParameterItem/1.0",
            "name": "flows",
            "type": 2,
            "description": "The business logic flow",
            "isRequired": true,
            "multiSelect": true,
            "quote": "'",
            "delimiter": ",",
            ...
            "jsonData": "[\r\n        {\"label\": \"Import CSV\", \"value\": \"REWARD_NOTIFICATION_FEEDBACK\", \"selected\": true},\r\n        {\"label\": \"Refund notification processing\", \"value\": \"PROCESS_REFUND\", \"selected\": true}\r\n]"
          },
          {
            "id": "3524e20a-4d4e-42a0-97da-13e703aad581",
            "version": "KqlParameterItem/1.0",
            "name": "innerSteps",
            "type": 2,
            "description": "The business logic inner steps",
            "isRequired": true,
            "multiSelect": true,
            "quote": "'",
            "delimiter": ",",
            ...
            "jsonData": "[\r\n        {\"label\": \"Feedback file download\", \"value\": \"{\\\"label\\\": \\\"Feedback file download\\\", \\\"value\\\": \\\"REWARD_NOTIFICATION_FEEDBACK_DOWNLOAD\\\"}\", \"selected\": true},\r\n        {\"label\": \"Total time per row\", \"value\": \"{\\\"label\\\": \\\"Total time per row\\\", \\\"value\\\": \\\"FEEDBACK_FILE_LINE_EVALUATION\\\"}\", \"selected\": true},\r\n        {\"label\": \"Wallet notification processing\", \"value\": \"{\\\"label\\\": \\\"Wallet notification processing\\\", \\\"value\\\": \\\"PROCESS_REFUND\\\"}\", \"selected\": true},\r\n        {\"label\": \"Feedback elaboration notification\", \"value\": \"{\\\"label\\\": \\\"Feedback elaboration notification\\\", \\\"value\\\": \\\"FEEDBACK_ELABORATION_NOTIFICATION\\\"}\", \"selected\": true}\r\n]"
          },
          {
            "id": "d00c7642-7980-4a8b-a2da-9e6ab440562e",
            "version": "KqlParameterItem/1.0",
            "name": "dbCollections",
            "type": 2,
            "isRequired": true,
            ...
            "jsonData": "[\r\n                    \"rewards_notification\",\r\n                    \"Timeline\",\r\n                    \"wallet\",\r\n                    \"rewards_organization_exports\",\r\n                    \"rewards_organization_imports\"\r\n]",
    ...
    },
    {
      "type": 1,
      "content": {
        "json": "The Executed Steps section show details and timings of the executed steps  ",
        "style": "info"
      },
      "name": "text - 7"
    },
        ...
        "items": [
          {
            "type": 1,
            "content": {
              "json": "WARN and ERRORS are obtained checking all the logs produced by the selected apps, thus it could contains alerts not related to the current logics",
              "style": "info"
            },
            "name": "text - 3"
          },
          ...
          },
          {
            "type": 10,
            "content": {
              "chartId": "workbook999ccfbf-083e-4978-89c8-ec6de3af281d",
              "version": "MetricsItem/2.0",
              "size": 1,
              "chartType": 2,
              "resourceType": "microsoft.eventhub/namespaces",
              ...
              "title": "Reward notification event toward citizen",
              "showOpenInMe": true,
              "filters": [
                {
                  "id": "1",
            ...
            "name": "Reward notification event toward citizen"
          },
          {
            "type": 1,
            "content": {
              "json": "In case of error due to IBAN not found, the citizen would be notified",
              "style": "info"
            },
            "name": "text - 2"
          }
        ...
        "items": [
          {
            ...
            "name": "Distribution of reward notification export's inner steps"
          },
          {
            "type": 3,
            "content": {
              "version": "KqlItem/1.0",
              "query": "//PERFORMANCE LOG: Distribution of reward notification export's inner steps average ms\r\n// Configuring timed window search\r\nlet startTime = {timeRange:start};\r\nlet endTime = {timeRange:end};\r\nlet interval = totimespan({timeSpan:label});\r\n// Configuring applications' logs to search\r\nlet apps = dynamic([{apps:value}]);\r\nlet flows = dynamic([{innerSteps:$.value}]);\r\nlet flowLabels = dynamic([{innerSteps:$.label}]);\r\n// Querying data\r\nlet startTimeRounded = bin(startTime, interval);\r\nlet endTimeRounded = bin(endTime, interval);\r\nlet FlowsMap = (arr: dynamic) {\r\n    toscalar (\r\n    range x from 0 to array_length(arr) - 1 step 1\r\n    | summarize x = make_list(strcat('[', arr[x], ']'))\r\n    )\r\n};\r\nlet data = ContainerLog\r\n| where TimeGenerated between (startTime .. endTime)\r\n| join kind=inner (\r\nKubePodInventory\r\n| where TimeGenerated between (startTime .. endTime) and Namespace == \"idpay\" and ServiceName has_any (apps)\r\n| distinct Computer, ContainerID, PodUid\r\n)//KubePodInventory Contains namespace information\r\non Computer, ContainerID\r\n| parse kind=regex flags=U LogEntry with \"^\" timestamp  \"\\\\s\\\\[\" appName \"\\\\]\\\\s\" level \"\\\\s+\\\\[\" thread \"\\\\]\\\\s\\\\[\" logger \"\\\\]\\\\s-\\\\s\" message \"$\"\r\n| project timestamp=datetime_local_to_utc(todatetime(timestamp), \"Europe/Rome\"), level, appName, PodUid, thread, logger, message\r\n| where message startswith \"[PERFORMANCE_LOG]\" and message has_any (FlowsMap(flows))\r\n| parse kind=regex flags=U message with \"^\\\\[PERFORMANCE_LOG\\\\] \\\\[\" flow \"\\\\] Time occurred to perform business logic: \" ms:int \" ms\"\r\n| where isnotnull(ms);\r\n// just intervals having data\r\n//data | summarize avg(ms) by appName, flow, bin(timestamp, interval)\r\n// intervals having data or empty appName\r\ndata | summarize avg(ms) by appName, flow, bin(timestamp, interval) | join kind=fullouter (range timestamp from startTimeRounded to endTimeRounded step interval) on timestamp | project timestamp=coalesce(timestamp, timestamp1), appName, flow, avg_ms\r\n// no appName distinction\r\n//data | make-series kind=nonempty avg(ms) on timestamp from startTimeRounded to endTimeRounded step interval\r\n| project timestamp, appName, flowIndex=array_index_of(flows, flow), avg_ms | project timestamp, flow=iff(flowIndex==-1,\"\",flowLabels[flowIndex]), appName, avg_ms\r\n| order by timestamp desc\r\n| render columnchart with(kind=unstacked, title=\"Distribution of reward notification export's inner steps average ms\")\r\n\r\n",
              "size": 0,
              "aggregation": 3,
              "showAnalytics": true,
              "title": "Distribution of reward notification export's inner steps average ms",
              "timeContextFromParameter": "timeRange",
              "timeBrushParameterName": "timeRange",
              "showExportToExcel": true,
              "queryType": 0,
            ...
            "name": "Distribution of reward notification export's inner steps average ms"
          },
          {
            "type": 3,
            "content": {
              "version": "KqlItem/1.0",
              "query": "//PERFORMANCE LOG: Distribution of PDV 429\r\n// Configuring timed window search\r\nlet startTime = {timeRange:start};\r\nlet endTime = {timeRange:end};\r\nlet interval = totimespan({timeSpan:label});\r\n// Configuring applications' logs to search\r\nlet apps = dynamic([{apps:value}]);\r\n// Querying data\r\nlet startTimeRounded = bin(startTime, interval);\r\nlet endTimeRounded = bin(endTime, interval);\r\nlet data = ContainerLog\r\n| where TimeGenerated between (startTime .. endTime)\r\n| join kind=inner (\r\nKubePodInventory\r\n| where TimeGenerated between (startTime .. endTime) and Namespace == \"idpay\" and ServiceName has_any (apps)\r\n| distinct Computer, ContainerID, PodUid\r\n)//KubePodInventory Contains namespace information\r\non Computer, ContainerID\r\n| parse kind=regex flags=U LogEntry with \"^\" timestamp  \"\\\\s\\\\[\" appName \"\\\\]\\\\s\" level \"\\\\s+\\\\[\" thread \"\\\\]\\\\s\\\\[\" logger \"\\\\]\\\\s-\\\\s\" message \"$\"\r\n| project timestamp=datetime_local_to_utc(todatetime(timestamp), \"Europe/Rome\"), level, appName, PodUid, thread, logger, message\r\n| where message startswith \"[PDV_INTEGRATION] Retrying invocation due to exception\";\r\n// just intervals having data\r\n//data | summarize avg(ms) by appName, bin(timestamp, interval)\r\n// intervals having data or empty appName\r\n//data | summarize count() by appName, bin(timestamp, interval) | join kind=fullouter (range timestamp from startTimeRounded to endTimeRounded step interval) on timestamp | project timestamp=coalesce(timestamp, timestamp1), appName, count_\r\n// intervals on all appNames, evenif no data \r\ndata | summarize count() by appName, bin(timestamp, interval) | join kind=fullouter (range timestamp from startTimeRounded to endTimeRounded step interval) on timestamp | project timestamp=coalesce(timestamp, timestamp1), flow=\"PDV Integration\", count_\r\n// no appName distinction\r\n//data | make-series kind=nonempty count() on timestamp from startTimeRounded to endTimeRounded step interval\r\n| order by timestamp desc\r\n| render columnchart with(kind=unstacked, title=\"Distribution of PDV 429\")",
              "size": 0,
              "showAnalytics": true,
              "title": "Distribution of PDV 429",
              "timeContextFromParameter": "timeRange",
              "showExportToExcel": true,
              "exportToExcelOptions": "all",
              "queryType": 0,
