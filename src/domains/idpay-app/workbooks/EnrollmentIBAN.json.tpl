  ...
  "items": [
    {
      "type": 1,
      "content": {
        "json": "### This workbook is intended to show the performance observed during the enrollment iban processing\r\n\r\nFor more functional information about the enrollment iban see:\r\nhttps://pagopa.atlassian.net/wiki/spaces/IDPAY/pages/504627600/Analisi+funzionale+dei+servizi+del+beneficiario#SB.-2.-Enrollment-dei-metodi-e-aggiunta-iban-da-APP-IO\r\n\r\nFor more technical information about the enrollment iban see: \r\nhttps://pagopa.atlassian.net/wiki/spaces/IDPAY/pages/506954092/Servizi+del+Beneficiario#API-%5BIssuer%5D---Enrollment-di-un-IBAN-%E2%80%93-Fase-di-Enrollment"
      },
      "name": "text - 2"
    },
    {
        ...
        "items": [
          {
            "type": 1,
            "content": {
              "json": "This section is dedicated to all kafka queues used in this flow.\r\nThe 2 graph show the incoming and outgoing messages from the eventhub ${prefix}-idpay-evh-ns-00 filtered by the folowing topics:\r\n\r\n1. idpay-checkiban-evaluation (see design rewiew [Wallet](https://pagopa.atlassian.net/wiki/spaces/IDPAY/pages/506954092/Servizi+del+Beneficiario#API---Enrollment-di-un-IBAN-%E2%80%93-Fase-di-Enrollment), [Iban](https://pagopa.atlassian.net/wiki/spaces/IDPAY/pages/506954092/Servizi+del+Beneficiario#Gestione-coda-check-Iban))\r\n2. idpay-checkiban-outcome (see design rewiew [Iban](https://pagopa.atlassian.net/wiki/spaces/IDPAY/pages/506954092/Servizi+del+Beneficiario#Gestione-coda-check-Iban), [Reward](https://pagopa.atlassian.net/wiki/spaces/IDPAY/pages/555320041/Rimborsi#Rimborsi%3A-Consumer-UserId-Iban))\r\n3. idpay-timeline - wallet sends the operation to Timeline\r\n4. idpay-notification-request (see design rewiew [Notification](https://pagopa.atlassian.net/wiki/spaces/IDPAY/pages/506954092/Servizi+del+Beneficiario#Ricezione-Esiti-KO-check-iban))\r\n\r\n"
            },
            "name": "text - 2"
          },
          {
      ...
      "content": {
        ...
        "items": [
          ...
          },
          {
            ...
            "content": {
              "version": "KqlItem/1.0",
              "query": "//PERFORMANCE LOG: Distribution of trx status\r\n// Configuring timed window search\r\nlet startTime = {timeRange:start};\r\nlet endTime = {timeRange:end};\r\nlet interval = totimespan({timeSpan:label});\r\n// Configuring applications' logs to search\r\nlet apps = dynamic(['idpay-iban']);\r\nlet flows = dynamic(['CHECK_IBAN_RESULT']);\r\n// Querying data\r\nlet startTimeRounded = bin(startTime, interval);\r\nlet endTimeRounded = bin(endTime, interval);\r\nlet FlowsMap = (arr: dynamic) {\r\n    toscalar (\r\n    range x from 0 to array_length(arr) - 1 step 1\r\n    | summarize x = make_list(strcat('[', arr[x], ']'))\r\n    )\r\n};\r\nlet data = ContainerLog\r\n| where TimeGenerated between (startTime .. endTime)\r\n| join kind=inner (\r\nKubePodInventory\r\n| where TimeGenerated between (startTime .. endTime) and Namespace == \"idpay\" and ServiceName has_any (apps)\r\n| distinct Computer, ContainerID, PodUid\r\n)//KubePodInventory Contains namespace information\r\non Computer, ContainerID\r\n| parse kind=regex flags=U LogEntry with \"^\" timestamp  \"\\\\s\\\\[\" appName \"\\\\]\\\\s\" level \"\\\\s+\\\\[\" thread \"\\\\]\\\\s\\\\[\" logger \"\\\\]\\\\s-\\\\s\" message \"$\"\r\n| project timestamp=datetime_local_to_utc(todatetime(timestamp), \"Europe/Rome\"), level, appName, PodUid, thread, logger, message\r\n| where message startswith \"[SAVE_IBAN]\" and message has_any (FlowsMap(flows));\r\ndata \r\n| summarize count() by result=case(message contains \"OK\", \"OK\", message contains \"UNKNOWN_PSP\", \"UNKNOWN_PSP\", \"KO\"), bin(timestamp, interval) \r\n| join kind=fullouter (range timestamp from startTimeRounded to endTimeRounded step interval) on timestamp \r\n| project timestamp=coalesce(timestamp, timestamp1), result, count_\r\n| sort by timestamp\r\n| render columnchart with(kind=unstacked, title=\"Distribution of checkiban's result\")",
              "size": 1,
              "showAnalytics": true,
              "title": "Distribution of CHECK_IBAN result",
              "timeContextFromParameter": "timeRange",
              "showExportToExcel": true,
            ...
            "customWidth": "50",
            "name": "Distribution of CHECK_IBAN result",
            "styleSettings": {
              "maxWidth": "50"
            }
          },
          ...
          },
          {
            "type": 1,
            "content": {
              "json": "CHECK_IBAN counts are performed on idpay-iban logs, thus it will be influeced by the number of messages read by that ms",
              "style": "warning"
            },
            "customWidth": "50",
