  ...
  "items": [
    {
      "type": 1,
      "content": {
        "json": "### This workbook is intended to show the performance observed during the onboarding workflow processing, specifically for citizens who aren't in the whitelist. \r\nFor more functional information about the onboarding workflow see:\r\nhttps://pagopa.atlassian.net/wiki/spaces/IDPAY/pages/504627600/Analisi+funzionale+dei+servizi+del+beneficiario#SB.-1-Processo-di-onboarding-del-beneficiario-da-APP-IO\r\n\r\nFor more technical information about the onboarding workflow see: \r\nhttps://pagopa.atlassian.net/wiki/spaces/IDPAY/pages/506954092/Servizi+del+Beneficiario#Onboarding-Workflow"
      },
      "name": "text - 2"
    },
    {
        ...
        "items": [
          {
            "type": 1,
            "content": {
              "json": "This section is dedicated to all kafka queues used in this flow.\r\nThe firt 3 graph show the incoming and outgoing messages from the eventhub ${prefix}-idpay-evh-ns-00 filtered by the folowing topics:\r\n\r\n1. idpay-onboarding-outcome (see design rewiew [Admissibility](https://pagopa.atlassian.net/wiki/spaces/IDPAY/pages/510722049/Rule+Engine+-+Valutatore+ammissibilit#Dipendenze.3), [Onboarding](https://pagopa.atlassian.net/wiki/spaces/IDPAY/pages/506954092/Servizi+del+Beneficiario#Ricezione-Esiti-richieste-di-Onboarding), and [Notification](https://pagopa.atlassian.net/wiki/spaces/IDPAY/pages/506954092/Servizi+del+Beneficiario#Ricezione-Esiti-richieste-di-Onboarding.3))\r\n2. idpay-timeline (see design rewiew [Timeline](https://pagopa.atlassian.net/wiki/spaces/IDPAY/pages/506954092/Servizi+del+Beneficiario#Ricezione-Esiti-richieste-di-Onboarding.2))\r\n\r\n\r\nThe last 3 graph show the incoming and outgoing messages from service bus filtered by the topic:\r\n\r\n1. idpay-onboarding-request (see design rewiew [Onboarding](https://pagopa.atlassian.net/wiki/spaces/IDPAY/pages/506954092/Servizi+del+Beneficiario#API---Salvataggio-Consensi---Fase-di-Adesione) and [Admissibility](https://pagopa.atlassian.net/wiki/spaces/IDPAY/pages/510722049/Rule+Engine+-+Valutatore+ammissibilit#Valutatore-Ammissibilit%C3%A0))\r\n"
            },
            "name": "text - 6"
          },
          {
        ...
        "items": [
          {
            ...
            "content": {
              ...
              "size": 1,
              "showAnalytics": true,
              "title": "Distribution of onboarding citizen processing",
              "timeContextFromParameter": "timeRange",
            ...
            "name": "Distribution of onboarding citizen processing"
          },
          {
            "type": 1,
            "content": {
          ...
          },
          {
            ...
            "content": {
              ...
              "size": 1,
              "showAnalytics": true,
              "title": "Distribution of single onboarding average ms",
              "timeContextFromParameter": "timeRange",
              "timeBrushParameterName": "timeRange",
              "showExportToExcel": true,
              "queryType": 0,
            ...
            "name": "Distribution of single onboarding average ms"
          },
          {
            "type": 1,
            "content": {
          ...
          },
          {
            ...
            "content": {
              "version": "KqlItem/1.0",
              "query": "//PERFORMANCE LOG: Distribution of trx status\r\n// Configuring timed window search\r\nlet startTime = {timeRange:start};\r\nlet endTime = {timeRange:end};\r\nlet interval = totimespan({timeSpan:label});\r\n// Configuring applications' logs to search\r\nlet apps = dynamic(['idpay-onboarding-workflow']);\r\nlet flows = dynamic(['RESULT']);\r\n// Querying data\r\nlet startTimeRounded = bin(startTime, interval);\r\nlet endTimeRounded = bin(endTime, interval);\r\nlet FlowsMap = (arr: dynamic) {\r\n    toscalar (\r\n    range x from 0 to array_length(arr) - 1 step 1\r\n    | summarize x = make_list(strcat('[', arr[x], ']'))\r\n    )\r\n};\r\nlet data = ContainerLog\r\n| where TimeGenerated between (startTime .. endTime)\r\n| join kind=inner (\r\nKubePodInventory\r\n| where TimeGenerated between (startTime .. endTime) and Namespace == \"idpay\" and ServiceName has_any (apps)\r\n| distinct Computer, ContainerID, PodUid\r\n)//KubePodInventory Contains namespace information\r\non Computer, ContainerID\r\n| parse kind=regex flags=U LogEntry with \"^\" timestamp  \"\\\\s\\\\[\" appName \"\\\\]\\\\s\" level \"\\\\s+\\\\[\" thread \"\\\\]\\\\s\\\\[\" logger \"\\\\]\\\\s-\\\\s\" message \"$\"\r\n| project timestamp=datetime_local_to_utc(todatetime(timestamp), \"Europe/Rome\"), level, appName, PodUid, thread, logger, message\r\n| where message startswith \"[COMPLETE_ONBOARDING]\" and message has_any (FlowsMap(flows));\r\ndata | summarize count() by result=case(message contains \"ONBOARDING_OK\", \"ONBOARDING_OK\", \"ONBOARDING_KO\"), bin(timestamp, interval) | join kind=fullouter (range timestamp from startTimeRounded to endTimeRounded step interval) on timestamp | project timestamp=coalesce(timestamp, timestamp1), result, count_\r\n| sort by timestamp\r\n| render columnchart with(kind=unstacked, title=\"Distribution of onboarding's result\")",
              "size": 1,
              "showAnalytics": true,
              "title": "Distribution of onboarding's result",
              "showExportToExcel": true,
              "exportToExcelOptions": "all",
              "queryType": 0,
              "resourceType": "microsoft.operationalinsights/workspaces",
            ...
            "customWidth": "50",
            "name": "Distribution of onboarding's result",
            "styleSettings": {
              "maxWidth": "50"
            }
          },
          ...
          },
          {
            "type": 1,
            "content": {
              "json": "ONBOARDING_OK and ONBOARDING_KO counts are performed on idpay-onboarding-workflow logs, thus it will be influeced by the number of messages read by that ms",
              "style": "warning"
            },
            "customWidth": "50",
