resource "azurerm_data_factory_pipeline" "hashpan_csv_pipeline" {
  ...
  activities_json = templatefile("pipelines/hashpan_csv_pipeline.json", {
    ...
    hashpan_csv_destination  = azurerm_data_factory_custom_dataset.hpans_blob_csv_destination.name
    binary_source_csv        = azurerm_data_factory_custom_dataset.binary_source_dataset.name
    binary_destination_zip   = azurerm_data_factory_custom_dataset.binary_destination_dataset.name
  })
}
